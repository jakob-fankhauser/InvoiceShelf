                ...
                <td width="50%" class="text-left header-section-left company-address-container company-address">
                    @if($estimate->company)
                        ...
                        @if($estimate->company->address)
                            @if($estimate->company->address->address_street_1)
                                {{ $estimate->company->address->address_street_1 }}
                                @if($estimate->company->address->address_street_2)
                                    {{ $estimate->company->address->address_street_2 }}
                                @endif
                                <br>
                            @endif
                            @if($estimate->company->address->zip || $estimate->company->address->city)
                                {{ $estimate->company->address->zip }} {{ $estimate->company->address->city }}<br>
                            @endif
                            @if($estimate->company->address->country)
                                Deutschland<br>
                            @endif
                        @endif
                    @endif
            ...
            <div class="customer-address-container">
                ...
                @if($estimate->customer)
                    ...
                    @if($estimate->customer->billingAddress)
                        @if($estimate->customer->billingAddress->address_street_1)
                            {{ $estimate->customer->billingAddress->address_street_1 }}<br>
                        @endif
                        @if($estimate->customer->billingAddress->zip || $estimate->customer->billingAddress->city)
                            {{ $estimate->customer->billingAddress->zip }} {{ $estimate->customer->billingAddress->city }}<br>
                        @endif
                        @if($estimate->customer->billingAddress->country)
                            @php
                                $countryCode = $estimate->customer->billingAddress->country->code ?? null;
                                $countryNamesDE = ['AT' => 'Österreich', 'DE' => 'Deutschland'];
                                $countryName = $countryNamesDE[$countryCode] ?? $estimate->customer->billingAddress->country->name ?? '';
                            @endphp
                            {{ $countryName }}
                        @endif
                    @endif
    ...
    <table width="100%">
        <tr>
            <td width="33%" style="vertical-align: top;">
                ...
                @if($estimate->company->address)
                    @if($estimate->company->address->address_street_1)
                        {{ $estimate->company->address->address_street_1 }}<br>
                    @endif
                    @if($estimate->company->address->zip || $estimate->company->address->city)
                        {{ $estimate->company->address->zip }} {{ $estimate->company->address->city }}<br>
                    @endif
